...
export default function Crops() {
  ...
  useEffect(() => {
    const fetchFarmers = async () => {
      try {
        ...
        if (!response.ok) throw new Error(`Failed to fetch farmers: ${response.status}`);
        const json = await response.json();
        if (!json.success) throw new Error("API returned an unsuccessful response");

        const cleanData = json.data
          .filter((row) => row.enterpriseType === "CROP")
          .map((row, idx) => ({
            id: `${row.farmid}-${idx}`,
  ...
  const regions = useMemo(() => [...new Set(allData.map((d) => d.region))], [allData]);
  const commodities = useMemo(() => [...new Set(allData.map((d) => d.commodity))], [allData]);
  ...
  const filteredData = useMemo(() => {
    return allData.filter((farmer) => {
      ...
      const matchesCommodity = commodityFilter === "" || farmer.commodity === commodityFilter;
  ...
  useEffect(() => setCurrentPage(0), [searchTerm, regionFilter, commodityFilter]);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto pt-10">
      <h2 className="text-2xl font-bold mb-6 text-green-700">
        Crop Farmers ({filteredData.length})
      </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {paginatedData.map((farmer) => (
          <div key={farmer.id} className="bg-white border rounded-xl p-5 shadow hover:shadow-lg transition">
            <h3 className="text-lg font-semibold text-green-700 mb-4">👨‍🌾 {farmer.name}</h3>
